    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .btn-back {
            ...
            text-decoration: none; border: 1px solid #e2e8f0; transition: all 0.15s;
        }
        ...
        .form-control, .form-select {
            ...
            color: #1e293b; padding: 9px 14px;
            background: #fafbfc; transition: all 0.18s;
        }
        ...
        .cat-preview {
            ...
            border-radius: 9px; font-size: 0.82rem; color: #1d4ed8;
        ...
        .btn-save {
            ...
            box-shadow: 0 4px 12px rgba(217,119,6,0.3); transition: all 0.2s;
        }
        .btn-save:hover {
            background: linear-gradient(135deg, #d97706, #b45309);
            transform: translateY(-1px); box-shadow: 0 6px 18px rgba(217,119,6,0.4);
        }
        .btn-cancel {
            display: inline-flex; align-items: center; gap: 6px;
        ...
        table.cat-tbl thead th {
            padding: 11px 18px; font-size: 0.71rem;
        ...
        .cat-desc-tbl { font-size: 0.78rem; color: #64748b; }
        .btn-select-cat {
        ...
        .btn-select-cat:hover { background: #dbeafe; color: #1d4ed8; }
...
<div class="main-content">
    ...
    <div class="topbar">
        <div>
            <div class="topbar-title">Chỉnh Sửa Sản Phẩm</div>
            <div class="topbar-breadcrumb">Trang chủ &rsaquo; Sản phẩm &rsaquo; Chỉnh sửa: {{ $product->name }}</div>
        </div>
    ...
    <div class="form-card">
        <div class="form-card-header">
            <div class="form-card-title">
                <div class="icon-wrap"><i class="fa-solid fa-pen-to-square"></i></div>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            @enderror
                            <!-- Preview danh mục đang chọn -->
                            <div class="cat-preview" id="catPreview">
                                <i class="fa-solid fa-circle-check"></i>
                                <span>Đã chọn: <strong id="catPreviewName"></strong></span>
                            </div>
                        </div>

                        <div class="mb-3">
                            <label class="field-label"><i class="fa-solid fa-copyright" style="color:#2563eb;font-size:0.75rem;"></i> Thương hiệu</label>
                            <input type="text" name="brand" class="form-control @error('brand') is-invalid @enderror" value="{{ old('brand', $product->brand) }}" placeholder="Ví dụ: Brembo, Ohlins">
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="attributes" rows="2" class="form-control @error('attributes') is-invalid @enderror" placeholder='{"Màu":"Đen","Dung tích":"500ml"}'>{{ old('attributes', $product->attributes ? json_encode($product->attributes, JSON_UNESCAPED_UNICODE) : '') }}</textarea>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="description" rows="3"
                            ...
                            placeholder="Nhập thông số kỹ thuật, xuất xứ...">{{ old('description', $product->description) }}</textarea>
    ...
    <div class="cat-table-card">
        <div class="cat-table-header">
            <div class="cat-table-title">
                ...
                Bảng phân loại danh mục
                <span style="background:#eff6ff;color:#2563eb;border-radius:20px;padding:2px 10px;font-size:0.72rem;font-weight:700;">
                    {{ $categories->count() }}
                </span>
            </div>
            <div class="tip-text">
                <i class="fa-solid fa-lightbulb" style="color:#f59e0b;"></i>
                Nhấn "Chọn" để áp dụng danh mục cho sản phẩm này
            </div>
        </div>
        ...
        <div class="table-responsive">
            <table class="cat-tbl" id="catTable">
                ...
                <tbody>
                    @foreach($categories as $cat)
                    <tr id="row-{{ $cat->id }}"
                        class="{{ $product->category_id == $cat->id ? 'selected-row' : '' }}">
                        <td>
                            <div class="id-pill">#{{ sprintf('%02d', $cat->id) }}</div>
                        </td>
                        <td>
                            <div class="cat-name-tbl">{{ $cat->name }}</div>
                        </td>
                        <td>
                            <div class="cat-desc-tbl">{{ Str::limit($cat->description, 80, '...') ?: '—' }}</div>
                        </td>
                        <td class="text-center">
                            <button type="button" class="btn-select-cat"
                                onclick="selectCategory(event, {{ $cat->id }}, @json($cat->name))">
...
<script>
    // @ts-nocheck
    // Hiện preview khi load trang nếu đã có danh mục
    window.addEventListener('DOMContentLoaded', function () {
        const sel = document.getElementById('categorySelect');
        updatePreview(sel.value, sel.options[sel.selectedIndex]?.dataset?.name);

        sel.addEventListener('change', function () {
            const opt = this.options[this.selectedIndex];
            updatePreview(this.value, opt?.dataset?.name);
            highlightRow(this.value);
        });
    });

    function updatePreview(val, name) {
    ...
    function selectCategory(event, id, name) {
        const sel = document.getElementById('categorySelect');
        sel.value = id;
        updatePreview(id, name);
        highlightRow(id);

        // Cập nhật nút trong bảng
        document.querySelectorAll('.btn-select-cat').forEach(b => {
            b.innerHTML = '<i class="fa-solid fa-hand-pointer"></i> Chọn';
        });
        event.target.closest('.btn-select-cat').innerHTML =
            '<i class="fa-solid fa-circle-check"></i> Đang dùng';
    }
</script>
</body>
